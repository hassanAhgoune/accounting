...
public class CpcSousClasseRest {
 ...
 @PostMapping("/")
	public ResponseEntity<?> create(@Valid @RequestBody CpcSousClasse cpcSousClasse, BindingResult result) {
		
		CpcSousClasse New = null;
		...
		try {
			New = cpcSousClasseService.save(cpcSousClasse);
...
public ResponseEntity<?> delete(@PathVariable Long id ) {
	...
	try {
		CpcSousClasse cpcSousClasse = cpcSousClasseService.findById(id);

	    cpcSousClasseService.delete(cpcSousClasse);
	} catch (DataAccessException e) {
		response.put("message", "Impossible d'interroger la base de données");
		response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
		return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
